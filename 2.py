...
# Initialize a set to store the sources where the image is found
image_sources = set()
...
for image_url, user_info in image_username_mapping.items():
    ...
    for (wx, wy, ww, wh) in website_faces:
        ...
        if similarity > threshold:
            ...
            image_sources[source].append(username)

# Print all the sources and usernames where the image is found
print('Image found in the following sources:')
for source, usernames in image_sources.items():
    if source == 'website1':
        print('Source: Facebook')
    elif source == 'website2':
        print('Source: Insta')
